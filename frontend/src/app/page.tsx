...
export default function DashboardPage() {
  ...
  // Helper to push logs to the monospace Terminal HUD console safely (renamed to resolve the variable collision)
  ...
  return (
    <div className="space-y-12 pb-32 text-[#39ff14] bg-[#020205] min-h-screen">
      
      {/* NATIVE CRT SCANLINES OVERLAY */}
      <MatrixOverlay />

      {/* HEADER BAR WITH FUNCTIONAL WALLET DISCOVERY */}
      <header className="sticky top-0 z-50 w-full border-b border-accent/20 bg-[#020205]/60 backdrop-blur-xl py-4 px-8 flex items-center justify-between">
        ...
        {connectedWallet ? (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-red-950/20 border border-red-500/30 text-xs font-mono font-bold text-red-400 hover:bg-red-900/30 transition-all duration-300"
            >
              DISCONNECT
            </button>
          </div>
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-[#020205] border border-accent/40 text-accent font-mono font-bold text-xs uppercase tracking-widest hover:bg-accent/10 hover:shadow-glow transition-all duration-300 disabled:opacity-50"
          >
      ...
      <div className="px-8 space-y-12">
        ...
        <section className="bg-[#05050c] border border-accent/25 rounded-3xl p-8 md:p-12 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-8 shadow-glow">
          ...
          <div className="max-w-2xl space-y-4">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded bg-accent/10 border border-accent/20 text-[10px] font-mono font-bold text-accent tracking-widest uppercase">
              ...
              VERIFICATION_STANDARDS: ACTIVE
            </div>
            <h1 className="text-3xl md:text-5xl font-mono font-black tracking-tight text-white leading-tight">
              AUTONOMOUS_TRUST <br />
          ...
          <div className="hidden md:block">
            ...
            <div className="w-48 h-48 rounded-2xl bg-[#020205] border border-accent/25 flex items-center justify-center p-6 relative shadow-glow">
              <div className="text-center space-y-2 font-mono">
                <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Total Scored Agents</p>
                <p className="text-4xl font-extrabold text-white">{jobs.length > 0 ? jobs.length : "2"}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5 space-y-8">
            ...
            {/* LOOKUP PANEL */}
            <div className="glass-panel border-accent/25 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-background/50 border border-border rounded-xl px-4 py-3 text-xs font-mono text-gray-100 placeholder-gray-700 focus:outline-none focus:border-accent transition-all duration-200"
                />
                <button
                  onClick={() => fetchAgentProfile(walletQuery)}
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-accent to-indigoAccent font-mono font-bold text-xs uppercase tracking-widest text-white hover:shadow-strongGlow transition-all duration-300"
                >
                  Fetch Telemetry & Audits
                </button>
              </div>
              {error && (
                <div className="mt-4 p-3 rounded-xl bg-red-950/20 border border-red-500/20 text-xs font-mono text-red-400">
                  {error}
                </div>
              )}
            </div>
            ...
            {/* ONBOARDING REGISTRY */}
            <div className="glass-panel border-accent/25 rounded-2xl p-6">
              <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-accent mb-4">Onboard Agent ID</h3>
                ...
                <div className="grid grid-cols-1 gap-4 font-mono">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-background/50 border border-border rounded-xl px-3.5 py-2.5 text-xs text-gray-200 focus:outline-none focus:border-accent"
                      required
                    />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={2}
                      className="w-full bg-background/50 border border-border rounded-xl px-3.5 py-2.5 text-xs text-gray-200 focus:outline-none focus:border-accent resize-none"
                    />
                  </div>
                </div>
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl border border-accent/40 bg-accent/5 font-mono font-bold text-xs uppercase tracking-wider text-accent hover:bg-accent/15 hover:shadow-glow transition-all duration-300"
                >
                  Register Agent ID Profile
                </button>
              </form>
              {regSuccess && (
                <div className="mt-4 p-3 rounded-xl bg-green-950/20 border border-green-500/20 text-xs font-mono text-green-400">
                  {regSuccess}
                </div>
              )}
            </div>
          ...
          <div className="lg:col-span-7 space-y-8">
            {profile ? (
              <div className="space-y-8">
                ...
                <div className="glass-panel border-accent/25 rounded-2xl p-6 relative overflow-hidden shadow-glow">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-white/5 pb-4 mb-4">
                    <div>
                      <h2 className="text-2xl font-mono font-black tracking-tight text-white">{profile.identity.name}</h2>
                      <span className="text-[10px] font-mono text-gray-500 tracking-wider uppercase block mt-1">ID: {profile.identity.id}</span>
                    </div>
                    <div className="self-start sm:self-center px-3 py-1 rounded bg-green-500/10 border border-green-500/20 text-[10px] font-mono font-bold text-green-400 tracking-widest uppercase">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {profile.identity.capabilities.map((c, i) => (
                      <span key={i} className="px-2.5 py-1 text-xs font-mono font-semibold text-gray-300 bg-white/5 border border-white/10 rounded">
                        {c}
                      </span>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  {/* TRUST SCORE GAUGE */}
                  <div className="glass-panel border-accent/25 rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden text-center shadow-glow">
                    <h4 className="text-[10px] font-mono font-bold uppercase tracking-widest text-accent mb-6">CovenantScore</h4>
                    ...
                    <div className="w-full grid grid-cols-2 gap-4 border-t border-white/5 pt-4 text-xs font-mono text-gray-400">
                      <div>
                  ...
                  {/* CREDIT SCORE GAUGE */}
                  <div className="glass-panel border-accent/25 rounded-2xl p-6 flex flex-col items-center justify-center relative overflow-hidden text-center shadow-glow">
                    <h4 className="text-[10px] font-mono font-bold uppercase tracking-widest text-indigoAccent mb-6">CovenantCredit</h4>
                ...
                {auditReasoning && (
                  <div className="bg-[#050811] border border-white/10 rounded-2xl p-6 relative overflow-hidden shadow-glow">
              ...
              <div className="glass-panel border-accent/25 rounded-2xl p-12 text-center text-gray-500">
                <p className="text-sm font-mono">Please select or register a valid CovenantID profile to display current on-chain rating telemetry.</p>
              </div>
            )}
          </div>
        ...
        <section className="glass-panel border-accent/25 rounded-3xl p-8 space-y-8 relative overflow-hidden shadow-glow">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-4 bg-background/30 border border-white/5 rounded-2xl p-5 font-mono">
    ...
    {/* MONOSPACE COMMAND TERMINAL INTERACTIVE HUD DRAWER */}
